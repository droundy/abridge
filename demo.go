...
func TestBids(handstring string, good, bad []string) int {
	...
	for _,g := range good {
		sc,expl,con := bridge.RateBid(h, g, bridge.DefaultConventions())
	...
	for _,b := range bad {
		sc,_,con := bridge.RateBid(h, b, bridge.DefaultConventions())
...
func main() {
	...
	ts := bridge.GetValidTables(bridge.South, "1C P1S P2S P", 100, bridge.DefaultConventions())
	fmt.Println(ts)
